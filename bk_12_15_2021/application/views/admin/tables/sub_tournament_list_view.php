  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-6 col-sm-6 col-xs-6 col-lg-6">
        <?php if ($nnid == 2) : ?>
          <a href="<?php echo base_url() ?>admin/Admin_Load_controller/subTournamentPubgRegForm/<?php echo $nnid; ?>" class="btn btn-info"><i class="fa fa-plus"></i>  ADD NEW </a>
        <?php else: ?>
          <a href="<?php echo base_url() ?>admin/Admin_Load_controller/subTournamentRegForm/<?php echo $nnid; ?>" class="btn btn-info"><i class="fa fa-plus"></i>  ADD NEW </a>
        <?php endif; ?>
      </div>  
      <?php $attributes = array('id' => 'demo-form2', 'class' => 'form-horizontal form-label-left', 'name' => 'myform', 'onsubmit' => 'return  validation()'); ?>
      <?php echo form_open_multipart('admin/Admin_Load_controller/searchSubgame', $attributes); ?>        
      <div class="col-md-6 col-sm-6 col-xs-6 col-lg-6">
        <div class=" input-append date form_datetime col-md-6 col-sm-6 col-xs-12">
          <input size="16" type="text" name="date" id="endDate" value="" readonly class="form-control col-md-7 col-xs-12"><span class="add-on"><i class="icon-th"></i></span>
        </div>
    ...
    <?php foreach ($sub_cotent as $row): ?>
      ...
      <tbody>
        <tr class="even pointer">                  
          ...
          <td class="text-center "><?php echo anchor('admin/edit/Admin_Edit_conrtroller/tournametListEdit/' . $newtablename . '/' . $id . '', 'EDIT <i class="fa fa-pencil-square-o" aria-hidden="true"></i>', 'class="btn btn-info"'); ?>  &nbsp; 
                                                     <?php echo anchor('admin/Admin_Load_controller/subtournametListAactivate/' . $newtablename . '/' . $id . '/' . $Name . '/' . $nnid . '', 'ACTIVE', 'class="btn btn-success"'); ?>  &nbsp; 
                                                     <?php echo anchor('admin/Admin_Load_controller/subtournametListDeactivate/' . $newtablename . '/' . $id . '/' . $Name . '/' . $nnid . '', 'DISABLE ', 'class="btn btn-default"'); ?>  &nbsp;</td>
