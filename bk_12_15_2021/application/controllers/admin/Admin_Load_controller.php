...
class Admin_Load_controller extends MY_Controller {
    ...
    public function addLeaderboardTournamentRegForm() {
        $this->load->model('admin/Get_model');
        $data['sub_cotent_tournament'] = $this->Get_model->getAllIteams('tournament');
        $data['sub_cotent_subtournament'] = $this->Get_model->getAllIteams('subtournament');
        $data['main_content'] = $this->load->view('admin/forms/leaderboard_tournament_add_form_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }

    public function editLeaderboardTournamentRegForm($id) {

        $this->load->model('admin/Get_model');
        $table = "leader_board";
        $data['sub_cotent_tournament'] = $this->Get_model->getAllIteams('tournament');
        $data['sub_cotent_subtournament'] = $this->Get_model->getAllIteams('subtournament');
        $data['sub_cotent_id'] = $this->Get_model->leaderBoardtListById($table, $id);
        $data['sub_cotent'] = $data['sub_cotent_id'][0];
        //echo '<pre>';print_r($data['sub_cotent']);die();
        $data['main_content'] = $this->load->view('admin/forms/leaderboard_tournament_edit_form_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }

    public function tournamentAssignForm($editor) {
        ...
        //echo '<pre>';print_r($data['editor']);die();

        $data['main_content'] = $this->load->view('admin/forms/tournament_assing_form_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }
    ...
    public function leaderBoardtListGroupByGame() {
        ...
        $data['sub_cotent'] = $this->Get_model->getAllIteams($TableName);
        $data['leader_board'] = $this->Get_model->getAllIteams('leader_board');
        $data['main_content'] = $this->load->view('admin/tables/leader_board_group_by_game_list_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }
    ...
    public function leaderBoardtListBySingleGame($id, $tournamentName) {
        ...
        $data['main_content'] = $this->load->view('admin/tables/leader_board_by_game_list_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }
    ...
    public function requestForTournamet() {

        $this->load->model('admin/Get_model');

        $TableName = "request_for_tournamet";
        $data['TableName'] = "request_for_tournamet";
        $data['sub_cotent'] = $this->Get_model->getAllIteams($TableName);

        //echo '<pre>';print_r($data['sub_cotent']);die();

        $data['main_content'] = $this->load->view('admin/tables/request_for_tournament_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }
    ...
    public function tournametListDetails($editor="") {
        ...
        if (empty($editor))
            ...
            $data['sub_cotent'] = $this->Get_model->getAllTournament($TableName);
    ...
    public function subtournametListDetails($tablename,$id, $isEditor="") {
        ...
        $data['Name'] = $gameName['0']['tournamentName'];
        if (!empty($isEditor))
            $data['sub_cotent'] =$this->Get_model->getDataForEditIndexDesending($newTable, $index,$Id, 'yes');
        else
    ...
    public function gameRegList($TableName,$gameName,$id, $isEditor="") {
        $this->load->model('admin/Get_model');
        ...
        if($TableName === "coc"){
            ...
            if ($isEditor=='')
                ...
                $pubgDetails = $this->Get_model->getDataForEditForEditor($searchTableName,$id);
            ...
            if ($isEditor=='')
                ...
                $pubgDetails = $this->Get_model->getDataForEditForEditor($searchTableName,$id);
            echo '<pre>';print_r($pubgDetails);die();
            //$pubgDetails = $this->Get_model->getDataForEdit($searchTableName,$id);
            $gamType = $pubgDetails[0]['gametype'];
    ...
    public function gameRegRuleLists($TableName,$gameName,$id) {
        $this->load->model('admin/Get_model');
        $ida = $id;
        $data['sub_cotent'] =  $this->Get_model->getSingleRule($ida);
        //echo '<pre>';print_r($data['sub_cotent']);die();
        if (empty($data['sub_cotent'])) {
            $data['main_content'] = $this->load->view('admin/forms/common_error_view','', TRUE);
            $this->load->view('admin/admin_mainTemplate_view', $data);
            return;
        }
        $data['rulesDescription'] = $data['sub_cotent'][0]['rulesDescription'];
        $data['table_id'] = $data['sub_cotent'][0]['id'];
        //echo '<pre>';print_r($data['rulesDescription']);die();
        $data['gamename'] = $data['sub_cotent'][0]['name'];
        $data['id'] = $ida;
        //$data['TableName'] = $TableName;
        //$data['id'] = $id;
        $data['main_content'] = $this->load->view('admin/tables/game_rule_lists_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);    
        
    }

    public function gameRegFixtureLists($TableName,$gameName,$id) {
        $this->load->model('admin/Get_model');
        $ida = $id;
        $data['sub_cotent'] =  $this->Get_model->getSingleFixture($ida);
        //echo '<pre>';print_r($data['sub_cotent']);die();
        if (empty($data['sub_cotent'])) {
            $data['main_content'] = $this->load->view('admin/forms/common_error_view','', TRUE);
            $this->load->view('admin/admin_mainTemplate_view', $data);
        ...
        $data['fixtureDescription'] = $data['sub_cotent'][0]['fixtureDescription'];
        $data['table_id'] = $data['sub_cotent'][0]['id'];
        //echo '<pre>';print_r($data['rulesDescription']);die();
        $data['gamename'] = $data['sub_cotent'][0]['name'];;
        $data['id'] = $ida;
        //$data['TableName'] = $TableName;
        //$data['id'] = $id;
        $data['main_content'] = $this->load->view('admin/tables/game_fixture_lists_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);    
        
    }
    
    public function userListView($editor='') {
    ...
    public function editorListView($editor='') {
        $this->load->library('pagination');
        $this->load->model('admin/Get_model');
        $TableName = "admin";   
        $totalrow = $this->Get_model->getAllRowCount($TableName);
        ...
        $data['TableName'] = "reguser";
        $data['sub_cotent'] = $this->Get_model->getAllIteamsPageignation($TableName, $config["per_page"], $page);
        $data['main_content'] = $this->load->view('admin/tables/editor_list_view', $data, TRUE);
        $this->load->view('admin/admin_mainTemplate_view', $data);
    }
